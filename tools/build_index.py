...
This is the NIGHTLY index.  It lists the same suite the stable index lists,
built from the branch rather than from a release -- so an entry here is
something to test, not something to settle into.  The stable index is
`wild1walker/Gen1Wild` and neither feed knows about the other; a player can add
both, and every nightly carries an id of its own so nothing here can replace
anything there.
...
which src/mods/ModIndex.lua resolves to
    ...
    https://wild1walker.github.io/Gen1NightlyIndex/data/index.json   (Pages)
    https://raw.githubusercontent.com/wild1walker/Gen1NightlyIndex/main/site/data/index.json
...
A nightly release is a PRERELEASE, which this resolver already handles: it
prefers a full release over a prerelease and takes a prerelease over nothing at
all -- and on this feed there is nothing else, so the newest nightly is what
every card resolves to.
...
OWNER, REPO = "wild1walker", "Gen1NightlyIndex"
...
def build():
    ...
    #
    # The bundle is the suite rather than a mod in it -- it installs most of
    # a playthrough's worth in a single card -- so it goes at the top instead
    # of falling wherever G sorts. Everything downstream reads the feed in
    # order: site/index.html draws it as given, and FIND MODS lists it as
    # given, so ordering here is the only place this has to be said.
